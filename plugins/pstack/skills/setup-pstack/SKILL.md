...
omp (Oh My Pi) has no sheet include and no second file. The live sheet is two `modelRoles` rows plus ten `task.agentModelOverrides` alias rows in `~/.omp/agent/config.yml`, and the ten `pstack-<stem>-<effort>` lane agents are its role surface. omp expands `@role` values in `agentModelOverrides` through `modelRoles`, so each family's selector lives in exactly one row and changing a panel model edits one line:

```text
modelRoles:
  pstack_fable: <provider>/<model>:<level>
  pstack_opus: <provider>/<model>:<level>
task:
  agentModelOverrides:
    pstack-fable-<effort>: "@pstack_fable:<level>"
    pstack-opus-<effort>: "@pstack_opus:<level>"
```

Every lane row carries an explicit `:level`. Each level is that lane's effort component clamped to the thinking levels the selected model actually serves (`omp models` lists them): nearest tier, ties upward, never a level outside the effort universe. The clamped level is what gets written, so the level read from the config is the level that runs. A model that serves none of the effort universe is a failed probe and writes nothing.
...
Read the current parent-specific sheet when it exists. Treat its values as current role-to-family assignments. Overlay its rows on the complete first-run role map in step 7. Materialize any missing documented role row from that map on the next successful write. A duplicate or unknown role row is inconsistent state; report it and resolve it before probing. A bare host-native slug from an older sheet is also invalid because it does not say which provider owns it. If the sheet is missing, use the complete first-run role map and the model matrix's Default effort cells. On omp, the current state is the two `pstack_fable` and `pstack_opus` rows under `modelRoles` plus the ten `pstack-*` rows under `task.agentModelOverrides`; every lane row must be an `"@pstack_<stem>:<level>"` alias whose level sits in the effort universe, and the two `modelRoles` rows must carry `provider/model:level` selectors. Legacy concrete lane rows (`<provider>/<model>:<level>` written directly) are valid loaded state from an earlier sheet: keep their family assignments and replace them with alias rows on the next successful write. A `pstack-*` lane row in any other shape is inconsistent state; stop and resolve it before probing.
...
Rewrite every matrix-family descriptor to `provider:model@<requested effort for that family>`. Leave `inherit-parent` and `auto` unchanged. An effort-only rerun cannot change a role's family. Changing Grok's effort updates every Grok occurrence and does not move a Sol role onto Grok. Refuse an unqualified slug, an unavailable route, a model other than the four matrix families, or a provider/model mismatch. On omp, render the two `modelRoles` rows plus the ten alias rows shown in the omp sheet format instead of the role list below: the lane name carries the role, each `modelRoles` row carries its family's selector with the family's clamped requested effort, and each lane alias's `:level` is that lane's effort component clamped to the model's served levels.
...
Render the parent integration in memory before either write. On Claude, the integration is the single `@~/.claude/pstack-models.md` include in `~/.claude/CLAUDE.md`. On Codex, it is the exact sheet bytes between one `<!-- pstack:models:begin -->` and `<!-- pstack:models:end -->` pair in `~/.codex/AGENTS.md`. On omp there is no integration artifact: `~/.omp/agent/config.yml` is live config, so the write itself is the wiring — snapshot the file, merge only the two `pstack_*` keys under `modelRoles` and the ten `pstack-*` alias rows under `task.agentModelOverrides`, and note that changes apply to new omp sessions. Replace both bounded sets on a rerun, converting any legacy concrete lane rows to alias rows. Insert them on first run. If either marker is missing, duplicated, or reversed, stop and report inconsistent state instead of guessing a boundary.
