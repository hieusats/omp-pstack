...
2. Pick the wake mechanism yourself. An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
...
5. Checkpoint every iteration via the **show-me-your-work** skill, a row for what changed and whether the predicate moved. A run with no trail can't be audited or resumed. Keep the todo list honest too: it starts with the Principles-read item per poteto-mode, phases get marked done as they land, and after a compaction rebuild it from `git log` and the decision trail before the next iteration.
