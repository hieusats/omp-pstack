...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch on omp's writer lane `pstack-task` with `isolated-write` in a dedicated worktree; review the diff. Capture a post-fix trace.
