...
- omp: dispatch `pstack-<stem>-<effort>` as native `task` agents — the same shipped definitions, discovered from the plugin's `agents/` directory. Model and effort resolution comes from `~/.omp/agent/config.yml`, written by the omp route of `setup-pstack` as two `modelRoles` rows (`pstack_fable`, `pstack_opus`) plus ten `"@pstack_<stem>:<level>"` alias rows under `task.agentModelOverrides`; omp expands the aliases through `modelRoles`, and each alias level is the lane's effort clamped to the model's served thinking levels, so the sheet shows the level that actually runs. omp silently runs a lane on the parent model when its mapping is missing, so treat an unmapped lane as unconfigured, never as `inherit-parent`. Fan out with one multi-item `task` dispatch; results arrive as background job results and are drained after fan-out.
