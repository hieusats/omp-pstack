...
This distribution targets omp only. Read [`references/provider-dispatch.md`](references/provider-dispatch.md) whenever a configured role launches. It defines the provider-qualified model descriptors, native/external route table, launcher, isolation, receipts, and dropout policy. Children never choose routes. Skill prose names omp tools directly (`read`, `write`, `edit`, `bash`, `grep`, `glob`, `task`, `todo`, `ask`); there is no cross-harness tool translation layer.
...
For `inherit-parent`, `auto`, or an unconfigured native ad-hoc helper, prefer `poteto-agent`. `/poteto-mode` and `poteto-agent` route through the same wrapper. A provider-qualified role instead follows provider dispatch: the shipped `pstack-<omp-role>` lane agents resolve model and effort through `task.agentModelOverrides`, and external providers run through the deterministic launcher. Routed workflow skills set the task and access mode; do not override their choices.

**Defaults for every delegation.** Start independent lanes together, use file pointers rather than inlined dumps, preserve only the tools or MCPs the task needs, and assign every writer a worktree or unique output directory. Depth is part of the contract: an inventory or summary assignment says what depth it needs and forbids full-corpus reads the answer does not require. An inventory works from names and structure, not file bodies: describe and it labels, exports, headings, directory shape; when a body must answer a specific doubt, open the range that answers, not the whole file. Size a `hub wait` timeout to the work you still owe after the result, and prefer a short wait plus a `hub jobs` re-check over one long blind block. `/setup-pstack` configures the lanes. Delegation defaults ride those lanes: code-writing goes to `pstack-task`, mechanical sweeps to `pstack-sonic`, exploration to `pstack-scout`, review and judgment to `pstack-reviewer` or `pstack-security-reviewer`, external research to `pstack-librarian`, design shaping to `pstack-designer`. Model and effort come from `task.agentModelOverrides`; an external provider-panel descriptor is an explicit opt-in for cross-provider signal, and upstream's family tiering is recorded in setup-pstack's role map. The panel skills enumerate their own panels. `inherit-parent` and `auto` use the parent model natively and reduce provider diversity when used in a panel.
