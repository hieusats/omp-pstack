...
Transcripts live under `~/.omp/agent/sessions/<encoded-cwd>/` for the active workspace (its cwd with every `/` turned into `-`), one JSONL file per chat. Every line is one chat message.
