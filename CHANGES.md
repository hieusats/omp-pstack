...
## 2.2.0 draws a status line from the lane sheet

The plugin ships its first runtime extension: `plugins/pstack/extension/index.js`, a single JSDoc-typed ES module with zero dependencies, registered through a minimal `plugins/pstack/package.json`. One silent `session_start` handler reads `task.agentModelOverrides` from `~/.omp/agent/config.yml` with a narrow indentation-window scan (node builtins only inside the read; no YAML parser, no timers), classifies the sheet, and publishes one ASCII line under the `pstack` status key, rendered when omp's `statusLine.showHookStatus` is on. A missing config file reads as `unconfigured` and points at setup-pstack; an unreadable one reads as `inconsistent` with a `config unreadable` fragment, because a setup run would fail on the same file. Unknown `pstack-*` keys, invalid selectors (the setup aliases `inherit-parent` and `auto` stay legal), and missing lanes surface as named fragments, two shown then `(+N more)`; the configured line lists the distinct lane selectors in lane order, three shown then `+N more`. `tests/check-pstack-status.test.ts` pins the literals, the parser edges, the read semantics, and the version triple; the distribution bumps to 2.2.0.
