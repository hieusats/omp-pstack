...
This record covers the whole port history. It began as Cursor → Claude Code substitutions, added a Codex build, then a 1.3.0 omp target, and from 2.0.0 omp is the only target. Sections below their release describe the tree as it was at that release; the 2.2.0 section describes the current tree.

## 2.2.0 gates the status line on poteto-mode activation

The plugin's runtime extension, `plugins/pstack/extension/index.js`, shrinks from a lane-sheet report to an activation gate: a single JSDoc-typed ES module with zero imports, still registered through the minimal `plugins/pstack/package.json`, that draws one line reading exactly `pstack: poteto-mode` — and draws nothing at all until poteto-mode has actually been activated in that session. The lane-sheet semantics die here: no config read, no YAML scan, no lane classification, and the `unconfigured`/`configured`/`inconsistent` renders are gone with them. Detection is substring-based on three boundary-guarded markers: a `before_agent_start` user turn carrying the skill-dispatch phrase (`User invoked the "poteto-mode" skill`), or a `tool_execution_start` whose JSON arguments read the skill (`skill://poteto-mode`, or a filesystem path ending in `skills/poteto-mode/SKILL.md`). Three silent handlers (`session_start`, `before_agent_start`, `tool_execution_start`) share one monotonic latch — activation has no off switch — and every later event re-asserts the line, so a status surface that arrives late still picks it up; `session_start` also rescans the session journal through the session manager when one is present, so a resumed session that already activated poteto-mode keeps its line. Prose that merely mentions poteto-mode, tool calls against other skills, and fresh sessions with no journal match nothing and draw nothing. The line renders when omp's `statusLine.showHookStatus` is on; a failed draw never interrupts the session. Tests pin the markers, the rejected false positives, the journal rescan, the wiring against a recording fake, and the unchanged 2.2.0 version homes.
...
Tracking issue [#19](https://github.com/hieusats/omp-pstack/issues/19), the follow-up to #17 that fixed `arena`. The same dead model-sheet branch survived in `swarm`, `how`, and `interrogate`, and `architect` still named quad runner defaults even though its Arena delegates resolution to the arena skill. Each skill now resolves its lanes from the live sheet, the `pstack-*` rows under `task.agentModelOverrides`, and dispatches native `task` runs of the `pstack-<omp-role>` agents so the sheet supplies model and effort. `swarm` defaults its workers to `pstack-task` (`pstack-designer` for design-shaped slices). `how` explores on `pstack-scout`, explains and synthesizes on `pstack-reviewer`, and critics default to `pstack-reviewer` plus `pstack-security-reviewer`.

## 2.1.6 resolves arena's panel from the omp lane sheet

Tracking issue [#17](https://github.com/hieusats/omp-pstack/issues/17). `arena` was still fanning out the first-run provider quad. Its Phase A and Phase C read `arena runners` and `arena cross-judge pool` rows "from the current harness's pstack model sheet when present", phrasing left over from the multi-harness era when `~/.claude/pstack-models.md` carried those rows. In omp the live sheet is `task.agentModelOverrides`, which holds exactly the seven `pstack-<omp-role>` lane keys, so the branch was structurally dead and every arena run silently dispatched the external claude, codex, and grok CLI lanes. Reproduced live: the sheet carries no `arena runners` row, `omp models` lists only zai so the quad has zero native routes, a grok-CLI quad leg returned an unauthenticated dropout, and the run collapsed to parent-only work. The skill now reads its panel from the sheet, `arena runners` maps to the writer lane `pstack-task` and `arena cross-judge pool` to the read-only `pstack-reviewer`, and the runner's model-matrix test pins the rewrite red-first.
...
Tracking issue [#7](https://github.com/hieusats/omp-pstack/issues/7). Each lane body now states its `tools` allowlist is a hard boundary and orders the lane to refuse a mutating assignment rather than route it through `hub` process starts, after a hostile dispatch to the read-only `pstack-librarian` lane reproduced the recorded escape: omp granted `hub op:"start"` beyond the `read, grep, glob, web_search` allowlist with no enforcement, and a `/bin/sh -c` payload wrote the probe file. The runner's model-matrix test pins the boundary sentence in the shared lane body, so an upstream sync cannot silently drop it. The grant is deliberate upstream behavior tracked as an enhancement at [can1357/oh-my-pi#7061](https://github.com/can1357/oh-my-pi/issues/7061); our report in [#10257](https://github.com/can1357/oh-my-pi/issues/10257) was closed as its duplicate. `AGENTS.md` gains the categorical source-edit rule (edit tool or `ast_edit`, never `sed -i` or ad-hoc patchers on tracked source) because agents violated it twice across the recorded sessions and AGENTS.md loads for every session in this repo. A new `tests/verify-installed-version.ts` turns the release-time install check into a rerunnable script: it compares the repo manifest version against `omp plugin list --json` and the newest marketplace cache directory, failing loudly on the downgrade and stale-cache shapes that surfaced during the 2.1.0 and 2.1.1 releases; `tests/verify-installed-version.test.ts` covers the pure comparison against the incident shapes. The Claude-ism field cleanup from 2.1.0 was re-proven by mutation (re-adding `background` and `disallowedTools` to a lane fails the roster pin), and the reverted role-alias experiment leaves no residue on main.
...
The omp port no longer needs a manual startup step. omp does not execute Claude Code `hooks.json`, so 1.2.x asked every user to paste the poteto-mode mandate into `~/.omp/agent/AGENTS.md`. omp's plugin rule discovery gives the port a native equivalent: the new `plugins/pstack/rules/pstack-session-mandate.md` carries `alwaysApply: true`, omp loads it from the installed plugin tree, and its full content is injected into every session's system prompt automatically. The rule mirrors `hooks/session-start-context.md` with omp's flat skill names (`skill://poteto-mode`, direct entry via `tdd`, `architect`, `how`, `why`, `arena`, `interrogate`), keeps the subagent escape clause, defers to user instructions, and stays addressable as `rule://pstack-session-mandate`. Verified live by probing a throwaway rule through a fresh `omp -p` session before committing to the mechanism, then again with the shipped rule after a local marketplace install.

The repository also ships `.omp-plugin/marketplace.json`, omp's preferred catalog path, byte-identical to `.claude-plugin/marketplace.json` (the equality is asserted on raw bytes); omp reads its copy, Claude Code keeps reading its own, and one shape serves both. The distribution version bumps to 1.3.0 so installed catalogs refresh. `tests/skill-collision-repro.sh` extends its version chain to the omp catalog, CI parses the new manifest, and a new `tests/check-omp-structure.test.ts` locks the structure: the two catalogs stay deep-equal, every manifest plus `UPSTREAM.md` share one version, the rule keeps its frontmatter and omp-flat mandate markers with no `pstack:` prefix leak, `docs/omp.md` documents the automatic mandate, and every `agents/*.md` carries the `name` plus `description` frontmatter omp's task-agent parser requires.

The external runner also accepts `--parent omp`. `provider-dispatch.md`'s route table already promised omp parents the external runner for non-native lanes, but the launcher's parent enum only knew `claude` and `codex`, so the documented omp route was unexecutable. The parent value feeds only the receipt and the same-provider rejection (which cannot trigger for omp), so widening the enum is the whole fix, locked by a red-first parse test.

## omp port: Oh My Pi (omp) as a third harness

omp-pstack ships the same shared skill tree to omp. omp reads the `.omp-plugin/marketplace.json` catalog (since 1.3.0; before that the `.claude-plugin/marketplace.json` fallback), discovers the skills under flat names (`/skill:architect`, `skill://poteto-mode`), and discovers the `agents/` directory as native task agents, so the plugin tree installs unmodified. Per the upstream rule that harness translation never forks a skill, three existing boundaries carry the omp behavior: `codex-tools.md` gains an omp tool-mapping section, `provider-dispatch.md` gains the omp route row and a native-lane note, and `setup-pstack` gains an omp parent route that renders the four family efforts as ten `task.agentModelOverrides` rows in `~/.omp/agent/config.yml` — omp's live model sheet, which outranks the lane agents' frontmatter `model:` and is written only after every selector passes a live registry probe. omp does not execute Claude Code `hooks.json`, so the SessionStart mandate is ported with flat skill names into `docs/omp.md` for a one-line copy into `~/.omp/agent/AGENTS.md`. Verified on omp 18.0.11: marketplace install, skill and agent discovery, selector probes (a valid selector prints its marker; an unknown one fails loudly), and a lane spawn observed running on its mapped model.

## 1.2.1 records Cursor pstack 0.14.5; make-bot-ui deliberately not ported

Two upstream pstack commits landed after the `bdf7aa3` sync point. `799151d` (#271) adds a `make-bot-ui` skill that teaches the agent to build a button page or dashboard which wakes a Cursor Grok Bot over a webhook routine; it registered the skill under `skills/grokbot/` and bumped upstream to 0.14.4. `6fecddb` (#275) moves the skill to `skills/make-bot-ui/` (registration at the skills root) and fixes the README row, bumping upstream to 0.14.5. The net upstream delta is that one skill, the README row, and the version field.

**Deliberately not ported.** The skill's every mechanic is a Cursor product surface with no Claude Code, Codex, or omp equivalent: it creates the webhook through Cursor's `update_state` tool (target `routine`, action `create`), reads the URL and sender key from the Routines panel behind Cursor's chat header (`Cmd+Shift+I`), posts to `api2.cursor.sh/automations/webhook/<id>`, requests the secret through a `SendToUser` `secret-request` card, and is woken as a Cursor `[routine]` turn carrying a `<webhook_event>` block. None of these has a mapping in `codex-tools.md` or a route in `provider-dispatch.md`, and the port's rule is that harness translation lives only at those boundaries and never forks a skill. This follows the standing precedent for Cursor automation surfaces: the Benny automations pack (0.10.0) and sticky mode are out for the same reason. The harness-neutral parts the skill also names (bind a local server to `0.0.0.0` for Tailscale peers, keep the sender key server-side, probe with a harmless payload before declaring the UI live) are ordinary engineering already covered by the existing skills. Revisit if any target harness grows a webhook- or routine-wake surface of its own, or if upstream publishes a harness-neutral variant of the skill.

The sync is therefore content-free for the shared skill tree: the tree is unchanged, `README-UPSTREAM.md` takes upstream's README verbatim (the one new row), the sync point in `UPSTREAM.md` moves to `6fecddba65801f9b9c08b8b328d998ee5b09d290`, `README.md` and `docs/reference.md` name the new point, and the distribution version bumps to 1.2.1 so installed catalogs refresh. No provenance rows change in `NOTICE.md`; nothing was imported.
