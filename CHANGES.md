...
The omp sheet render is alias-based. `setup-pstack` writes two `modelRoles` rows (`pstack_fable`, `pstack_opus`) plus ten `"@pstack_<stem>:<level>"` rows under `task.agentModelOverrides`; omp expands the aliases through `modelRoles`, so each family's selector lives in one row. Each lane level is the lane effort clamped to the thinking levels `omp models` reports for the selected model, nearest tier with ties upward, and the clamped value is what gets written. A model serving none of the effort universe fails the probe and writes nothing. Reruns convert legacy concrete rows to alias rows. omp silently clamps unservable levels at dispatch and the registry can serve a subset of the effort universe (zai glm-5.3 serves `low`, `high`, `max`), so a per-effort concrete render either fails setup or hides the clamp; the alias sheet makes the model and the served level visible in config.
