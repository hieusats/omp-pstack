...
Tracking issue [#19](https://github.com/hieusats/omp-pstack/issues/19), the follow-up to #17 that fixed `arena`. The same dead model-sheet branch survived in `swarm`, `how`, and `interrogate`, and `architect` still named quad runner defaults even though its Arena delegates resolution to the arena skill. Each skill now resolves its lanes from the live sheet, the `pstack-*` rows under `task.agentModelOverrides`, and dispatches native `task` runs of the `pstack-<omp-role>` agents so the sheet supplies model and effort. `swarm` defaults its workers to `pstack-task` (`pstack-designer` for design-shaped slices). `how` explores on `pstack-scout`, explains and synthesizes on `pstack-reviewer`, and critics default to `pstack-reviewer` plus `pstack-security-reviewer`, extended by `pstack-librarian` when external prior art matters. `interrogate`'s reviewer table becomes `pstack-reviewer`, `pstack-security-reviewer`, and `pstack-librarian`, and its synthesis says when same-provider lanes reduced the cross-model signal. `architect` drops its runner sentence and defers to arena's Phase A. External provider-panel values are explicit opt-ins everywhere, an unconfigured sheet fails closed to setup-pstack, and `blast-radius` plus `figure-it-out` reword their arena references from models to lanes. No skill restates the quad any more, so `skill-collision-repro.sh` checks it only across provider dispatch and setup-pstack's reference rows, and `model-matrix.test.ts` pins the four resolutions red-first. `README.md` and `docs/reference.md` describe the new panel behavior.
