...
**Maintenance.** The plugin version string now lives in three manifests: `plugins/pstack/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, and `plugins/pstack/.codex-plugin/plugin.json`. A version bump must update all three; `tests/skill-collision-repro.sh` checks they match. `.agents/plugins/marketplace.json` carries no version field. The canonical default panel quad lives in `provider-dispatch.md` and is copied into the four panel skills (`arena`, `architect`, `how`, `interrogate`) and the `setup-pstack` sheet. Keep those copies grep-identical when models change; `tests/skill-collision-repro.sh` checks them against the canonical dispatch table. After a sync that touches `skills/poteto-mode/scripts/`, run `bun install --frozen-lockfile`, `bun run test`, and `bun run typecheck` from that directory. `hooks/session-start-context.md` restates skill one-liners — re-verify it whenever skill names or descriptions change. Every `commands/*.md` must carry `disable-model-invocation: true` (see 0.9.7) — `grep -L 'disable-model-invocation: true' plugins/pstack/commands/*.md` must print nothing, especially after an upstream sync adds a command. The mirror also holds (see 0.9.8): no skill may carry `disable-model-invocation` in its frontmatter — on a skill it makes the Skill tool refuse the invocation, breaking both the SessionStart mandate and the trampoline's "invoke the skill" body. The 21 command-less `principle-*` leaves instead carry `user-invocable: false` (see 0.9.9) to stay out of the `/` menu while `poteto-mode` reads them by path: `grep -L 'user-invocable: false' plugins/pstack/skills/principle-*/SKILL.md` must print nothing, and no leaf may also carry `disable-model-invocation` (the pair cancels to a dead skill). Re-run `tests/skill-collision-repro.sh` after Claude Code upgrades; the resolution precedence it depends on is undocumented, and the script now checks the static invariants: the command, skill, and leaf flags; version parity across the three manifests; and the default model quad's identity across the dispatch reference, four panel skills, and `setup-pstack`.
